...
public class ConfigCustomUserConverter implements UserAuthenticationConverter {
    ...
    /** ini untuk nge-ekstractdata authentication dan credentials **/
    @Override
    public Authentication extractAuthentication(Map<String, ?> map) {
        if (map.containsKey(USERNAME))
            return new UsernamePasswordAuthenticationToken(
                    new CustomPrincipal(map.get(USERNAME).toString(),
                            map.get(EMAIL).toString()), "N/A",
                    getAuthorities(map));
        return null;
    }
}
